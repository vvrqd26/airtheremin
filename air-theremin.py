...
class AirTheremin:
    ...
    def __init__(self):
        ...
        self.frequency_smoothing_rate = 0.1  # Sliding speed
    ...
    def process_frame(self, frame):
        ...
        if self.hands_detected:
            for hand_landmarks, hand in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                if hand_type == 'Right':
                    # 大拇指和食指之间的距离控制音量
                    finger_distance = self.calculate_finger_distance(
                        hand_landmarks)

                    # 根据手指距离映射音量
                    # 假设手指靠近时距离很小（<0.1），完全张开时距离较大（>0.5）
                    self.volume = max(0, min(1, (finger_distance - 0.1) / 0.4))

        cv2.putText(
            frame,
            f'''
{self.current_note} {self.target_frequency:.0f}Hz Vol:{self.volume:.2f}''',
            (int(w/2) - 150, int(h/2)),
    ...
    # 在Y轴 画音阶刻度线 根据 self.note_freqs 画出音阶刻度线
